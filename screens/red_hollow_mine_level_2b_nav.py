"""
Red Hollow Mine - Level 2B Navigation
Dead end spider lair with high combat density
"""
...
from data.maps.red_hollow_mine_level_2b_map import (
    MINE_L2B_WIDTH,
    MINE_L2B_HEIGHT,
    MINE_L2B_SPAWN_X,
    MINE_L2B_SPAWN_Y,
...
class RedHollowMineLevel2BNav:
    ...
    def __init__(self):
        config = {
            'map_width': MINE_L2B_WIDTH,
            'map_height': MINE_L2B_HEIGHT,
            'location_id': 'red_hollow_mine_level_2b',
            'map_functions': {
    ...
    def update_player_position(self, game_state):
        ...
        self.renderer.update_camera(game_state.mine_l2b_x, game_state.mine_l2b_y)
    ...
    def update(self, dt, keys, game_state, controller=None):
        ...
        # Handle movement
        old_x = game_state.mine_l2b_x
        old_y = game_state.mine_l2b_y
        new_x, new_y = self.renderer.handle_movement(keys, old_x, old_y)

        if new_x != old_x or new_y != old_y:
            # Check for combat trigger (HIGH DENSITY AREA)
            ...
            if combat_trigger and combat_trigger.get('repeatable'):
                ...
                if random.random() < chance:
                    if controller:
                        game_state.previous_screen = 'red_hollow_mine_level_2b_nav'
                        game_state.pre_combat_location = 'red_hollow_mine_level_2b_nav'  
            ...
            game_state.mine_l2b_x = new_x
            game_state.mine_l2b_y = new_y
        ...
        if keys[pygame.K_RETURN] and not self.showing_message:
            player_x = game_state.mine_l2b_x
            player_y = game_state.mine_l2b_y
            ...
            if searchable_info:
                ...
                if flag_set and getattr(game_state, flag_set, False):
                    ...
                    if examine_dialogue and controller:
                        ...
                        setattr(game_state, return_screen_attr, 'red_hollow_mine_level_2b_nav')
...
_mine_level_2b_instance = None

def draw_red_hollow_mine_level_2b_nav(surface, game_state, fonts, images, controller=None):
    """ScreenManager integration function"""
    global _mine_level_2b_instance

    if _mine_level_2b_instance is None:
