"""
Dungeon Level 1 Navigation - Ancient Upper Ruins
First level of the cult's sanctum
"""

import pygame
import random
from ui.base_location_navigation import NavigationRenderer
from utils.constants import (BLACK, WHITE, YELLOW, LAYOUT_DIALOG_Y, 
...
from data.maps.dungeon_level_1_map import (
    DUNGEON_L1_WIDTH,
    DUNGEON_L1_HEIGHT,
    DUNGEON_L1_SPAWN_X,
    DUNGEON_L1_SPAWN_Y,
...
from data.maps.dungeon_level_2_map import DUNGEON_L2_SPAWN_POINTS

class DungeonLevel1Nav:
    """Navigation screen for dungeon level 1"""
    ...
    def __init__(self):
        ...
        try:
            self.tilemap = load_tiled_map_with_names(
                'dungeon_level_1_tiles',  # TMJ filename (without extension)
                'dungeon',
                DUNGEON_TILE_MAP,
                TILESETS_PATH
            )
            ...
            self.tilemap = {
                'width': DUNGEON_L1_WIDTH,
                'height': DUNGEON_L1_HEIGHT,
                'tile_grid': None
            }
        ...
        config = {
            ...
            'spawn_position': (DUNGEON_L1_SPAWN_X, DUNGEON_L1_SPAWN_Y),
            'spawn_direction': 'up'
        }
    ...
    def update_player_position(self, game_state):
        ...
        if hasattr(game_state, 'dungeon_l1_spawn_override_x'):
            game_state.dungeon_l1_x = game_state.dungeon_l1_spawn_override_x
            game_state.dungeon_l1_y = game_state.dungeon_l1_spawn_override_y
            delattr(game_state, 'dungeon_l1_spawn_override_x')
            delattr(game_state, 'dungeon_l1_spawn_override_y')
            print(f"✅ Level 1 spawn override: ({game_state.dungeon_l1_x}, {game_state.dungeon_l1_y})")
        elif not hasattr(game_state, 'dungeon_l1_x'):
            game_state.dungeon_l1_x = DUNGEON_L1_SPAWN_X
            game_state.dungeon_l1_y = DUNGEON_L1_SPAWN_Y
            print("🏛️ Entered Dungeon Level 1")

        self.renderer.update_camera(game_state.dungeon_l1_x, game_state.dungeon_l1_y)
    ...
    def update(self, dt, keys, game_state, controller=None):
        ...
        # Handle movement
        old_x = game_state.dungeon_l1_x
        old_y = game_state.dungeon_l1_y
        ...
        if new_x != old_x or new_y != old_y:
            ...
            if combat_trigger:
                ...
                if completion_flag and getattr(game_state, completion_flag, False):
                    ...
                    game_state.dungeon_l1_x = new_x
                    game_state.dungeon_l1_y = new_y
                    self.renderer.update_camera(new_x, new_y)
                    return

                # Trigger combat
                if controller:
                    game_state.previous_screen = 'dungeon_level_1_nav'
                    game_state.pre_combat_location = 'dungeon_level_1_nav'
                    ...
                    controller.event_manager.emit("SCREEN_CHANGE", {
                        ...
                        "source_screen": "dungeon_level_1_nav"
                    })
                return
        ...
        if (self.renderer.check_enter_just_pressed(keys) and 
            ...
            player_x = game_state.dungeon_l1_x
            player_y = game_state.dungeon_l1_y
            ...
            if transition_info and transition_info[0]:
                if controller:
                    ...
                    if target == 'dungeon_level_2_nav':
                        spawn_point = DUNGEON_L2_SPAWN_POINTS['from_level_1']
                        game_state.dungeon_l2_spawn_override_x = spawn_point[0]
                        game_state.dungeon_l2_spawn_override_y = spawn_point[1]
                    elif target == 'hill_ruins_ground_level_nav':
                        # Set spawn for returning to hill ruins
                        game_state.hill_ruins_ground_spawn_override_x = 7
                        game_state.hill_ruins_ground_spawn_override_y = 11
            ...
            if searchable_info:
                ...
                if flag_set and getattr(game_state, flag_set, False):
                    ...
                    if examine_dialogue and controller:
                        ...
                        setattr(game_state, return_screen_attr, 'dungeon_level_1_nav')
...
_dungeon_level_1_instance = None

def draw_dungeon_level_1_nav(surface, game_state, fonts, images, controller=None):
    """ScreenManager integration function"""
    global _dungeon_level_1_instance

    if _dungeon_level_1_instance is None:
        _dungeon_level_1_instance = DungeonLevel1Nav()

    if hasattr(pygame, 'key') and pygame.get_init():
        keys = pygame.key.get_pressed()
        dt = 16
        _dungeon_level_1_instance.update(dt, keys, game_state, controller)

    return _dungeon_level_1_instance.render(surface, fonts, images, game_state)
