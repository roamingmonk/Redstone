"""
Dungeon Level 5 Navigation - Portal Chamber
Final confrontation area with Marcus and the final boss
"""
...
from data.maps.dungeon_level_5_map import (
    DUNGEON_L5_WIDTH,
    DUNGEON_L5_HEIGHT,
    DUNGEON_L5_SPAWN_X,
    DUNGEON_L5_SPAWN_Y,
...
class DungeonLevel5Nav:
    """Navigation screen for dungeon level 5 - Portal Chamber"""

    def __init__(self):
        config = {
            'map_width': DUNGEON_L5_WIDTH,
            'map_height': DUNGEON_L5_HEIGHT,
            'location_id': 'dungeon_level_5',
            'map_functions': {
    ...
    def update_player_position(self, game_state):
        ...
        if hasattr(game_state, 'dungeon_l5_spawn_override_x'):
            game_state.dungeon_l5_x = game_state.dungeon_l5_spawn_override_x
            game_state.dungeon_l5_y = game_state.dungeon_l5_spawn_override_y
            delattr(game_state, 'dungeon_l5_spawn_override_x')
            delattr(game_state, 'dungeon_l5_spawn_override_y')
            print(f"✅ Level 5 spawn override: ({game_state.dungeon_l5_x}, {game_state.dungeon_l5_y})")
        elif not hasattr(game_state, 'dungeon_l5_x'):
            game_state.dungeon_l5_x = DUNGEON_L5_SPAWN_X
            game_state.dungeon_l5_y = DUNGEON_L5_SPAWN_Y
            print("⚫ Entered the Portal Chamber - Level 5")

        self.renderer.update_camera(game_state.dungeon_l5_x, game_state.dungeon_l5_y)
    ...
    def update(self, dt, keys, game_state, controller=None):
        ...
        # Handle movement
        old_x = game_state.dungeon_l5_x
        old_y = game_state.dungeon_l5_y
        ...
        if new_x != old_x or new_y != old_y:
            ...
            if combat_trigger:
                ...
                # Trigger combat
                if controller:
                    game_state.previous_screen = 'dungeon_level_5_nav'
...
def draw_dungeon_level_5_nav(surface, game_state, fonts, images, controller=None):
    ...
    global _dungeon_level_5_instance

    if _dungeon_level_5_instance is None:
        _dungeon_level_5_instance = DungeonLevel5Nav()

    if hasattr(pygame, 'key') and pygame.get_init():
        keys = pygame.key.get_pressed()
        dt = 16
        _dungeon_level_5_instance.update(dt, keys, game_state, controller)

    return _dungeon_level_5_instance.render(surface, fonts, images, game_state)