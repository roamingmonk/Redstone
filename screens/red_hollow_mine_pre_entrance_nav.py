"""
Red Hollow Mine - Pre-Entrance Area Navigation
Safe zone with Henrik's seal and warnings
"""
...
class RedHollowMinePreEntranceNav:
    """Navigation screen for mine pre-entrance area"""

    def __init__(self):
        config = {
            'map_width': MINE_PRE_WIDTH,
            'map_height': MINE_PRE_HEIGHT,
            'location_id': 'red_hollow_mine_pre_entrance',
            'map_functions': {
    ...
    def update_player_position(self, game_state):
        ...
        if hasattr(game_state, 'mine_pre_spawn_override_x'):
            game_state.mine_pre_x = game_state.mine_pre_spawn_override_x     
            game_state.mine_pre_y = game_state.mine_pre_spawn_override_y 
            delattr(game_state, 'mine_pre_spawn_override_x')
            delattr(game_state, 'mine_pre_spawn_override_y')
            print(f"✅ Pre-entrance spawn override: ({game_state.mine_pre_x}, {game_state.mine_pre_y})")
        elif not hasattr(game_state, 'mine_pre_x'):
            game_state.mine_pre_x = MINE_PRE_SPAWN_X
            game_state.mine_pre_y = MINE_PRE_SPAWN_Y
        
        self.renderer.update_camera(game_state.mine_pre_x, game_state.mine_pre_y)
    ...
    def update(self, dt, keys, game_state, controller=None):
        ...
        # Handle movement
        old_x = game_state.mine_pre_x
        old_y = game_state.mine_pre_y
        new_x, new_y = self.renderer.handle_movement(keys, old_x, old_y)

        if new_x != old_x or new_y != old_y:
            game_state.mine_pre_x = new_x
        ...
        if keys[pygame.K_RETURN] and not self.showing_message:
            ...
            if transition_info and transition_info[0]:
                ...
                can_transition = True
                blocked_message = transition_data.get('blocked_message', 'You cannot proceed.')
                ...
                if flags_must_be_true:
                    any_true = any(getattr(game_state, flag_name, False) for flag_name in flags_must_be_true)
                    if not any_true:
                        can_transition = False
                ...
                if can_transition and controller:
                    ...
                    if target == 'red_hollow_mine_level_1_nav':
                        spawn_point = MINE_L1_SPAWN_POINTS['from_pre_entrance']
            ...
            if searchable_info:
                ...
                if flag_set and getattr(game_state, flag_set, False):
                    ...
                    if examine_dialogue and controller:
                        ...
                        setattr(game_state, return_screen_attr, 'red_hollow_mine_pre_entrance_nav')
    ...
    def _trigger_loot_check(self, loot_table_id, game_state, controller):
        ...
        # Convert to loot overlay format
        items_list = []
        ...
        game_state.pre_combat_location = 'red_hollow_mine_pre_entrance_nav'
    ...
    def render(self, surface, fonts, images, game_state):
        ...
        title_text = "RED HOLLOW MINE - ENTRANCE"
...
_mine_pre_entrance_instance = None

def draw_red_hollow_mine_pre_entrance_nav(surface, game_state, fonts, images, controller=None):
    """ScreenManager integration function"""
    global _mine_pre_entrance_instance

    if _mine_pre_entrance_instance is None:
